...
- A denied tool call is a recoverable control transition, not completion or blockage of the Goal by itself. Read controller `status.next_action` and continue through the legal recovery path; only a valid checkpoint, explicit user decision, or genuine external blocker may terminate progress.
